...
void GLWall::SplitUpperEdge(FFlatVertex *&ptr, float *lightuv, float lindex)
{
...
void GLWall::SplitLowerEdge(FFlatVertex *&ptr, float *lightuv, float lindex)
{
	side_t *sidedef = seg->sidedef;
	float polyw = glseg.fracright - glseg.fracleft;
...
void GLWall::SplitLeftEdge(FFlatVertex *&ptr, float *lightuv, float lindex)
{
...
void GLWall::SplitRightEdge(FFlatVertex *&ptr, float *lightuv, float lindex)
{
...
int GLWall::CreateVertices(FFlatVertex *&ptr, bool split)
{
	...
	ptr++;
	if (split && !(flags & GLWF_NOSPLITUPPER && seg->sidedef->numsegs > 1)) SplitUpperEdge(ptr, lightuv, lindex);
	...
	ptr++;
	if (split && !(flags & GLWF_NOSPLITLOWER) && seg->sidedef->numsegs > 1) SplitLowerEdge(ptr, lightuv, lindex);
	return int(ptr - oo);
}
