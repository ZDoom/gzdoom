...
struct slot_t
{
	...
	int SLL;    // Sustin Level (ajusted) = volume oů l'enveloppe termine sa premiere phase de regression
	int KSR_S;  // Key Scale Rate Shift = facteur de prise en compte du KSL dans la variations de l'enveloppe
	int KSR;    // Key Scale Rate = cette valeur est calculee par rapport ŕ la frequence actuelle, elle va influer
	...
	int Finc;   // frequency step = pas d'incrementation du compteur-frequence
				// plus le pas est grand, plus la frequence est aďgu (ou haute)
	int Ecurp;  // Envelope current phase = cette variable permet de savoir dans quelle phase
				// de l'enveloppe on se trouve, par exemple phase d'attaque ou phase de maintenue ...
				// en fonction de la valeur de cette variable, on va appeler une fonction permettant
				// de mettre ŕ jour l'enveloppe courante.
	int Ecnt;   // Envelope counter = le compteur-enveloppe permet de savoir oů l'on se trouve dans l'enveloppe
	int Einc;   // Envelope step courant
	int Ecmp;   // Envelope counter limite pour la prochaine phase
	int EincA;  // Envelope step for Attack = pas d'incrementation du compteur durant la phase d'attaque
				// cette valeur est egal ŕ AR[KSR]
	int EincD;  // Envelope step for Decay = pas d'incrementation du compteur durant la phase de regression
				// cette valeur est egal ŕ DR[KSR]
	int EincS;  // Envelope step for Sustain = pas d'incrementation du compteur durant la phase de maintenue
				// cette valeur est egal ŕ SR[KSR]
	int EincR;  // Envelope step for Release = pas d'incrementation du compteur durant la phase de rel'chement
				// cette valeur est egal ŕ RR[KSR]
	int *OUTp;  // pointeur of SLOT output = pointeur permettant de connecter la sortie de ce slot ŕ l'entree
				// d'un autre ou carrement ŕ la sortie de la voie
...
struct state_t
{
	...
	int TimerA;         // timerA limit = valeur jusqu'ŕ laquelle le timer A doit compter
	int TimerAL;
	int TimerAcnt;      // timerA counter = valeur courante du Timer A
	int TimerB;         // timerB limit = valeur jusqu'ŕ laquelle le timer B doit compter
	int TimerBL;
...
void Ym2612_Impl::run( int pair_count, Ym2612_Emu::sample_t* out )
{
	...
	// Mise ŕ jour des pas des compteurs-frequences s'ils ont ete modifies
	...
	for ( int chi = 0; chi < channel_count; chi++ )
	{
		...
		for ( int i = 0; i < 4; i++ )
		{
			...
			{                       // les differents taux pour l'enveloppe sont mis ŕ jour
				sl.KSR = ksr;
