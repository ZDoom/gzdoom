...
void alphaBlend(uint32_t& dst, uint32_t col) //blend color over destination with opacity N / M
{
    ...
    dst = ((((col >> 24) * N + (dst >> 24) * (M - N) ) / M) << 24) |
          (redMask   & ((col & redMask  ) * N + (dst & redMask  ) * (M - N)) / M) | //this works because 8 upper bits are free
...
FORCE_INLINE
bool sPixDoLineBlend(const Kernel_3x3& ker, const char blend, const xbrz::ScalerCfg& cfg)
{
