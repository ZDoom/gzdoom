...
void P_KillMobj (AActor *source, AActor *target, AActor *inflictor)
{
	...
	if (target->player)
	{
		...
		if (bglobal.botnum && consoleplayer == Net_Arbitrator && !demoplayback)
		{
			if (target->player->isbot)
				target->player->t_respawn = (P_Random(pr_botrespawn)%15)+((bglobal.botnum-1)*2)+TICRATE+1;

			//Added by MC: Discard enemies.
			for (int i = 0; i < MAXPLAYERS; i++)
			{
				if (players[i].isbot && target == players[i].enemy)
				{
	...
	//Added by MC: Optional weapon dropping.
    case MT_PLAYER:
		if (weapondrop.value && target->player)
			item = bglobal.Warrior_weapon_drop (target);
		break;
